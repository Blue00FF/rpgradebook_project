...
@dataclass
class Student:
    """The attributes of this class describe a student at University."""
    ...
    def __post_init___(self):
        ...
        if self.email is None:
            self.email = f"{self.first_name.lower()}.{self.last_name.lower()}@univ.edu"

        assert (
            len(str(self.psid))
        ) == 7, f"PSID not 7 digits for {self.first_name} {self.last_name}"
...
students = []
N_STUDENTS = 150
students.extend(Student() for _ in range(N_STUDENTS))
